...
Chat.prototype.getHistory = function (channelName, callback) {
    if (channelName === 'all')
        db.query.getAllChatTable(CHAT_HISTORY_SIZE, fn);
    else
        db.query.getChatTable(channelName,CHAT_HISTORY_SIZE, fn);
...
Chat.prototype.saveChatMessage = function (userId, message, channelName, isBot, date) {
    db.query.addChatMessage(userId, date, message, channelName, isBot, function (err) {
        if (err)
...
Chat.prototype.mute = function (shadow, moderatorUser, username, time, channelName, callback) {
    ...
    db.query.getUserByName(username, function (err, userInfo) {
